...
async def test_can_create_memory_via_consolidator(memory_tools):
    ...
    result = await memory_tools.save_memory(title="Test Memory", content="This is test content")
...
async def test_can_search_via_consolidator(memory_tools):
    ...
    await memory_tools.save_memory(title="Test Memory", content="This is test content about Python")
...
async def test_can_tag_via_consolidator(memory_tools):
    ...
    mem = await memory_tools.save_memory(title="Test", content="Content")

    result = await memory_tools.add_tag(memory_id=mem["memory_id"], tag_name="Python")
...
async def test_can_relate_via_consolidator(memory_tools):
    ...
    mem1 = await memory_tools.save_memory(title="Memory 1", content="First memory")
    mem2 = await memory_tools.save_memory(title="Memory 2", content="Second memory")
