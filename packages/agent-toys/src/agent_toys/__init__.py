...
Each MCP app is registered as a resource or tool provider.
"""
...
# ==================== IMPORT TOOLS FROM MEM-LITE ====================

# Import all tools from mem-lite
from mem_lite_mcp.server import (
    save_memory,
    update_memory,
    remove_memory,
    search_memory,
...
# Register mem-lite tools on agent-toys (they're already decorated with @mem_lite_app.tool())
# We need to re-wrap them for agent-toys app
...
# ==================== MEMORY TOOLS (from mem-lite-mcp) ====================
...
async def mem_save_memory(
    ...
    """[mem-lite] Save a new memory with auto-generated ULID identifier.
...
async def mem_update_memory(
    ...
    """[mem-lite] Update title, content, or summary of an existing memory.
...
async def mem_search_memory(
    ...
    """[mem-lite] Search memories with full-text search, relation depth, and importance scoring.
...
@app.tool()
async def mem_get_memory(
    memory_ids: List[str] = Field(description="List of memory IDs to retrieve", min_items=1, max_items=50),
    tools: MemoryTools = Depends(get_memory_tools)
) -> dict:
    """[mem-lite] Retrieve multiple memories with complete content and metadata.
...
async def mem_add_tag(
    ...
    """[mem-lite] Add a tag to an existing memory.
...
async def mem_add_relation(
    ...
    """[mem-lite] Create a bidirectional relationship between two memories.
...
# ==================== MEMORY PROMPTS (from mem-lite-mcp) ====================

from fastmcp.prompts import Message, PromptResult


@app.prompt(
    name="mem_maintenance",
    description="[mem-lite] Comprehensive memory maintenance cycle (consolidate, compress, deduplicate, optimize, quality check)",
    tags={"maintenance"}
)
def mem_maintenance_prompt() -> PromptResult:
    """[mem-lite] Complete memory maintenance operation in a single pass.
