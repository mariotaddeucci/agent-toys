...
class Database:
    ...
    async def init_db(self):
        ...
        self.engine = create_async_engine(
            self.db_url, echo=False, future=True, connect_args={"check_same_thread": False}
        )
    ...
    async def save_memory(
        self, memory_id: str, title: str, content: str, summary: str | None = None, tags: list[str] | None = None
    ) -> MemoryRead:
    ...
    async def get_memory_by_id(self, memory_id: str) -> MemoryRead | None:
        async with await self.get_session() as session:
            memory = await session.execute(select(Memory).where(Memory.id == memory_id))
    ...
    async def update_memory(
        self, memory_id: str, title: str | None = None, content: str | None = None, summary: str | None = None
    ) -> bool:
        async with await self.get_session() as session:
            memory = await session.execute(select(Memory).where(Memory.id == memory_id))
            memory = memory.scalar_one_or_none()
    ...
    async def delete_memory(self, memory_id: str) -> dict:
        async with await self.get_session() as session:
            relations = await session.execute(
                select(func.count(Relation.id)).where(or_(Relation.id_a == memory_id, Relation.id_b == memory_id))
            )
            ...
            return {"tags_removed": 0, "relations_removed": relations_removed}
    ...
    async def update_last_read(self, memory_ids: list[str]):
        ...
        async with await self.get_session() as session:
            now = get_now_timestamp()
            memories = await session.execute(select(Memory).where(Memory.id.in_(memory_ids)))
    ...
    async def add_relation(self, memory_id_1: str, memory_id_2: str, weight: float = 0.5) -> Relation:
        ...
        async with await self.get_session() as session:
            ...
            existing = await session.execute(select(Relation).where(and_(Relation.id_a == id_a, Relation.id_b == id_b)))
            ...
            if existing:
                ...
                relation = Relation(id=relation_id, id_a=id_a, id_b=id_b, weight=weight, created_at=now)
                session.add(relation)

            await session.commit()

            return Relation(id=relation_id, id_a=id_a, id_b=id_b, weight=weight, created_at=now)
    ...
    async def get_related_memories(self, memory_id: str, depth: int = 1) -> list[dict]:
        async with await self.get_session() as session:
            ...
            relations = await session.execute(
                select(Relation).where(or_(Relation.id_a == memory_id, Relation.id_b == memory_id))
            )
            ...
            for rel in relations:
                ...
                if mem:
                    related.append({"other_id": mem.id, "weight": rel.weight, "distance": 1})
            ...
            if depth > 1:
                ...
                if direct_ids:
                    indirect_relations = await session.execute(
                        select(Relation).where(or_(Relation.id_a.in_(direct_ids), Relation.id_b.in_(direct_ids)))
                    )
                    ...
                    for rel in indirect_relations:
                        ...
                        if other_id not in direct_ids and other_id != memory_id:
                            mem = await self.get_memory_by_id(other_id)
                            if mem:
                                related.append({"other_id": mem.id, "weight": rel.weight, "distance": 2})
